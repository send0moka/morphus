...
## Internal Local App
For office/internal rollout, package Morphus Converter so each user runs the converter on their own laptop without installing Node.js. The default package is slim and uses the system browser; set `MORPHUS_BUNDLE_BROWSER=1` only when you need a fully offline Chromium bundle. GitHub Actions can build the macOS and Windows zips plus a Figma plugin zip for you, and macOS users can install through Homebrew Cask after a GitHub Release is published. See [docs/local-companion.md](docs/local-companion.md).
